...
class PSTDescriptorItem {
	PSTDescriptorItem(byte[] data, int offset, PSTFile pstFile) {
		...
		if (pstFile.getPSTFileType() == PSTFile.PST_TYPE_ANSI) {
			descriptorIdentifier = (int) PSTObject
					.convertLittleEndianBytesToLong(data, offset, offset + 4);
			offsetIndexIdentifier = ((int) PSTObject
					.convertLittleEndianBytesToLong(data, offset + 4,
							offset + 8)) & 0xfffffffe;
			subNodeOffsetIndexIdentifier = (int) PSTObject
					.convertLittleEndianBytesToLong(data, offset + 8,
			...
			offsetIndexIdentifier = ((int) PSTObject
					.convertLittleEndianBytesToLong(data, offset + 8,
							offset + 16)) & 0xfffffffe;
			subNodeOffsetIndexIdentifier = (int) PSTObject
					.convertLittleEndianBytesToLong(data, offset + 16,
	...
	public int[] getBlockOffsets() throws IOException, PSTException {
		...
		Long[] offsets = pstFile.readLeaf(offsetIndexIdentifier)
				.getBlockOffsets();

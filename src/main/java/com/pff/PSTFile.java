...
public class PSTFile {
	 ...
	 *             read error
	 */
	public PSTFile(String fileName) throws FileNotFoundException, PSTException,
			IOException {
		this(new File(fileName));
	}

	public PSTFile(File fileName) throws FileNotFoundException, PSTException,
			IOException {
		...
		try {
			...
			if (!strValue.equals("!BDN")) {
				throw new PSTException("Invalid file header: " + strValue
						+ ", expected: !BDN");
			}
			...
			if (fileTypeBytes[0] != PSTFile.PST_TYPE_ANSI
				...
				throw new PSTException("Unrecognised PST File version: "
						+ fileTypeBytes[0]);
			}
	 ...
	 *             file error
	 */
	private void processNameToIdMap(RandomAccessFile in) throws IOException,
			PSTException {
		...
		for (int i = 0; i < nGuids; ++i) {
			long mostSigBits = (PSTObject.convertLittleEndianBytesToLong(guids,
					offset, offset + 4) << 32)
					| (PSTObject.convertLittleEndianBytesToLong(guids,
							offset + 4, offset + 6) << 16)
					| PSTObject.convertLittleEndianBytesToLong(guids,
							offset + 6, offset + 8);
			long leastSigBits = PSTObject.convertBigEndianBytesToLong(guids,
					offset + 8, offset + 16);
		...
		byte[] stringNameToIdByte = getData(stringMapEntries,
				localDescriptorItems);

		// process the entries
		for (int x = 0; x + 8 < nameToIdByte.length; x += 8) {
			int dwPropertyId = (int) PSTObject.convertLittleEndianBytesToLong(
					nameToIdByte, x, x + 4);
			int wGuid = (int) PSTObject.convertLittleEndianBytesToLong(
					nameToIdByte, x + 4, x + 6);
			int wPropIdx = ((int) PSTObject.convertLittleEndianBytesToLong(
					nameToIdByte, x + 6, x + 8));
			...
			if ((wGuid & 0x0001) == 0) {
				...
				nameToId.put((long) dwPropertyId | ((long) guidIndex << 32),
						wPropIdx);
				...
				int len = (int) PSTObject.convertLittleEndianBytesToLong(
						stringNameToIdByte, dwPropertyId, dwPropertyId + 4);
				byte[] keyByteValue = new byte[len];
				System.arraycopy(stringNameToIdByte, dwPropertyId + 4,
						keyByteValue, 0, keyByteValue.length);
				wPropIdx += 0x8000;
	...
	private byte[] getData(PSTTableItem item,
		...
		PSTDescriptorItem mapDescriptorItem = localDescriptorItems
	...
	static String getInternetCodePageCharset(int propertyId) {
		if (bCPFirstTime) {
			...
			try {
				InputStream propertyStream = PSTFile.class
						.getResourceAsStream("/InternetCodepages.txt");
	...
	static String getPropertyName(int propertyId, boolean bNamed) {
		if (bFirstTime) {
			...
			try {
				InputStream propertyStream = PSTFile.class
						.getResourceAsStream("/PropertyNames.txt");
				if (propertyStream != null) {
	...
	static String getPropertyDescription(int entryType, int entryValueType) {
		...
		if (entryType < 0x8000) {
			...
			if (type == -1) {
				ret = String.format("0xFFFF(%04X):%04X: ", entryType,
						entryValueType);
			} else {
				String name = PSTFile.getPropertyName((int) type, true);
				if (name != null) {
					ret = String.format("%s(%04X):%04X: ", name, entryType,
							entryValueType);
				} else {
					ret = String.format("0x%04X(%04X):%04X: ", type, entryType,
							entryValueType);
				}
			}
		}
	 ...
	 *             file error
	 */
	private byte[] findBtreeItem(RandomAccessFile in, long index,
			boolean descTree) throws IOException, PSTException {
		...
		while ((temp[0] == 0xffffff80 && temp[1] == 0xffffff80 && !descTree)
			...
			if (levelsToLeaf > 0) {
				...
				for (int x = 0; x < numberOfItems; x++) {
					if (this.getPSTFileType() == PST_TYPE_ANSI) {
						long indexIdOfFirstChildNode = extractLEFileOffset(btreeStartOffset
								+ (x * 12));
						if (indexIdOfFirstChildNode > index) {
							// get the address for the child first node in this
							// group
							btreeStartOffset = extractLEFileOffset(btreeStartOffset
									+ ((x - 1) * 12) + 8);
							in.seek(btreeStartOffset + 500);
							in.read(temp);
							found = true;
				...
				if (!found) {
					...
					if (this.getPSTFileType() == PST_TYPE_ANSI) {
						btreeStartOffset = extractLEFileOffset(btreeStartOffset
								+ ((numberOfItems - 1) * 12) + 8);
						in.seek(btreeStartOffset + 500);
						in.read(temp);
					} else {
						btreeStartOffset = extractLEFileOffset(btreeStartOffset
								+ ((numberOfItems - 1) * 24) + 16);
						in.seek(btreeStartOffset + 496);
						in.read(temp);
					}
				}
	 ...
	 *             file error
	 */
	OffsetIndexItem getOffsetIndexNode(long identifier) throws IOException,
			PSTException {
	 ...
	 * 
	 * @return HashMap<Integer, PSTDescriptorItem>
	 */
	HashMap<Integer, PSTDescriptorItem> getPSTDescriptorItems(
			long localDescriptorsOffsetIndexIdentifier) throws PSTException,
			IOException {
		return this.getPSTDescriptorItems(this
				.readLeaf(localDescriptorsOffsetIndexIdentifier));
	}

	HashMap<Integer, PSTDescriptorItem> getPSTDescriptorItems(
			PSTNodeInputStream in) throws PSTException, IOException {
	 ...
	 * @throws PSTException
	 *             file error
	 * @return LinkedHashMap<Integer, LinkedList<DescriptorIndexNode>>
	 */
	...
	private void processDescriptorBTree(long btreeStartOffset)
		...
		if ((temp[0] == 0xffffff81 && temp[1] == 0xffffff81)) {
			...
			if (levelsToLeaf > 0) {
				for (int x = 0; x < numberOfItems; x++) {
					if (this.getPSTFileType() == PST_TYPE_ANSI) {
						...
						long nextLevelStartsAt = this
								.extractLEFileOffset(branchNodeItemStartIndex + 8);
						processDescriptorBTree(nextLevelStartsAt);
					} else {
						long branchNodeItemStartIndex = (btreeStartOffset + (24 * x));
						long nextLevelStartsAt = this
								.extractLEFileOffset(branchNodeItemStartIndex + 16);
						processDescriptorBTree(nextLevelStartsAt);
					}
				}
			} else {
				for (int x = 0; x < numberOfItems; x++) {
					...
					DescriptorIndexNode tempNode = new DescriptorIndexNode(
							temp, this.getPSTFileType());

					// we don't want to be children of ourselves...
					if (tempNode.parentDescriptorIndexIdentifier == tempNode.descriptorIdentifier) {
						// skip!
					} else if (childrenDescriptorTree
							.containsKey(tempNode.parentDescriptorIndexIdentifier)) {
			...
			throw new PSTException(
					"Unable to read descriptor node, is not a descriptor");
		}
	}

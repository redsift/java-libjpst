...
public class PSTDistList extends PSTMessage {
	 ...
	 */
	private byte[] oneOffEntryIdUid = { (byte) 0x81, (byte) 0x2b, (byte) 0x1f,
			(byte) 0xa4, (byte) 0xbe, (byte) 0xa3, (byte) 0x10, (byte) 0x19,
			(byte) 0x9d, (byte) 0x6e, (byte) 0x00, (byte) 0xdd, (byte) 0x01,
			(byte) 0x0f, (byte) 0x54, (byte) 0x02 };

	/**
	 * identifier for wrapped entries.
	 */
	private byte[] wrappedEntryIdUid = { (byte) 0xc0, (byte) 0x91, (byte) 0xad,
			(byte) 0xd3, (byte) 0x51, (byte) 0x9d, (byte) 0xcf, (byte) 0x11,
			(byte) 0xa4, (byte) 0xa9, (byte) 0x00, (byte) 0xaa, (byte) 0x00,
	...
	private OneOffEntry parseOneOffEntry(byte[] data, int pos)
			throws IOException {
		int version = (int) PSTObject.convertLittleEndianBytesToLong(data, pos,
				pos + 2);
		pos += 2;
		...
		System.arraycopy(data, pos, displayNameBytes, 0,
		...
		System.arraycopy(data, pos, addressTypeBytes, 0,
				addressTypeBytes.length);
		String addressType = new String(addressTypeBytes, "UTF-16LE");
		pos = stringEnd + 2;

		stringEnd = findNextNullChar(data, pos);
		byte[] emailAddressBytes = new byte[stringEnd - pos];
		System.arraycopy(data, pos, emailAddressBytes, 0,
				emailAddressBytes.length);
	 ...
	 */
	public Object[] getDistributionListMembers() throws PSTException,
			IOException {
		PSTTableBCItem item = this.items.get(pstFile.getNameToIdMapItem(0x8055,
		...
		if (item != null) {
			...
			pos += 4;
			pos = (int) PSTObject.convertLittleEndianBytesToLong(item.data,
					pos, pos + 4);
			...
			for (int x = 0; x < count; x++) {
				...
				if (Arrays.equals(guid, wrappedEntryIdUid)) {
					...
					pos++;
					int wrappedflags = (int) PSTObject
							.convertLittleEndianBytesToLong(item.data, pos,
									pos + 4);
					pos += 4;
					...
					int descriptorIndex = (int) PSTObject
							.convertLittleEndianBytesToLong(item.data, pos,
									pos + 3);
					pos += 3;
					...
					out[x] = PSTObject.detectAndLoadPSTObject(this.pstFile,
							descriptorIndex);

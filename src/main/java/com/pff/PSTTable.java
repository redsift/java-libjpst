...
class PSTTable {
	...
	protected PSTTable(PSTNodeInputStream in,
		...
		default:
			throw new PSTException(
					"Unable to parse table, bad table type.  Unknown identifier: 0x"
		...
		if (headerByte != 0xb5) {
			...
			throw new PSTException(
					"Unable to parse table, can't find BTHHEADER header information: "
							+ headerByte);
		}

		sizeOfItemKey = (int) headerNodeInfo.in.read() & 0xFF; // Size of key in
																// key table
		sizeOfItemValue = (int) headerNodeInfo.in.read() & 0xFF; // Size of
																	// value in
																	// key table
		...
		description += "Table (" + tableType + ")\n" + "hidUserRoot: "
				+ hidUserRoot + " - 0x" + Long.toHexString(hidUserRoot) + "\n"
				+ "Size Of Keys: " + sizeOfItemKey + " - 0x"
				+ Long.toHexString(sizeOfItemKey) + "\n" + "Size Of Values: "
				+ sizeOfItemValue + " - 0x" + Long.toHexString(sizeOfItemValue)
				+ "\n" + "hidRoot: " + hidRoot + " - 0x"
	...
	class NodeInfo {
		...
		NodeInfo(int start, int end, PSTNodeInputStream in) throws PSTException {
			if (start > end)
				throw new PSTException(
						String.format(
								"Invalid NodeInfo parameters: start %1$d is greater than end %2$d",
								start, end));
			startOffset = start;
			endOffset = end;
			this.in = in;
		...
		long seekAndReadLong(long offset, int length) throws IOException,
				PSTException {
	...
	protected NodeInfo getNodeInfo(int hnid) throws PSTException, IOException {
		...
		if (subNodeDescriptorItems != null
			...
			try {
				// data = item.getData();
				PSTNodeInputStream subNodeIn = new PSTNodeInputStream(
						in.getPSTFile(), item);
				subNodeInfo = new NodeInfo(0, (int) subNodeIn.length(),
						subNodeIn);
			} catch (IOException e) {
				throw new PSTException(String.format(
						"IOException reading subNode: 0x%08X", hnid));
			}
		...
		if (whichBlock > this.arrayBlocks.length) {
			...
			String err = String.format(
					"getNodeInfo: block doesn't exist! hnid = 0x%08X\n", hnid);
			err += String.format(
					"getNodeInfo: block doesn't exist! whichBlock = 0x%08X\n",
					whichBlock);
		...
		int iHeapNodePageMap = (int) in.seekAndReadLong(blockOffset, 2)
				+ blockOffset;
		int cAlloc = (int) in.seekAndReadLong(iHeapNodePageMap, 2);
		if (index >= cAlloc + 1) {
			throw new PSTException(String.format(
					"getNodeInfo: node index doesn't exist! nid = 0x%08X\n",
					hnid));
			// return null;
		}
		iHeapNodePageMap += (2 * index) + 2;
		int start = (int) in.seekAndReadLong(iHeapNodePageMap, 2) + blockOffset;
		int end = (int) in.seekAndReadLong(iHeapNodePageMap + 2, 2)
				+ blockOffset;

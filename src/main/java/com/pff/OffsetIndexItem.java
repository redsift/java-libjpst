...
class OffsetIndexItem {
	...
	OffsetIndexItem(byte[] data, int pstFileType) {
		if (pstFileType == PSTFile.PST_TYPE_ANSI) {
			indexIdentifier = PSTObject.convertLittleEndianBytesToLong(data, 0,
					4);
	...
	public String toString() {
		return "OffsetIndexItem\n" + "Index Identifier: " + indexIdentifier
				+ " (0x" + Long.toHexString(indexIdentifier) + ")\n"
				+ "File Offset: " + fileOffset + " (0x"
				+ Long.toHexString(fileOffset) + ")\n" + "cRef: " + cRef
				+ " (0x" + Long.toHexString(cRef) + " bin:"
				+ Long.toBinaryString(cRef) + ")\n" + "Size: " + size + " (0x"

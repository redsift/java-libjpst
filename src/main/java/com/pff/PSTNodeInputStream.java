...
public class PSTNodeInputStream extends InputStream {
	...
	PSTNodeInputStream(PSTFile pstFile, PSTDescriptorItem descriptorItem)
		...
		// with
		OffsetIndexItem offsetItem = pstFile
				.getOffsetIndexNode(descriptorItem.offsetIndexIdentifier);
	...
	private void getBlockSkipPoints(byte[] data) throws IOException,
			PSTException {
		if (data[0] != 0x1) {
			throw new PSTException(
					"Unable to process XBlock, incorrect identifier");
		}

		int numberOfEntries = (int) PSTObject.convertLittleEndianBytesToLong(
				data, 2, 4);
		...
		if (data[1] == 0x2) {
			...
			for (int x = 0; x < numberOfEntries; x++) {
				long bid = PSTObject.convertLittleEndianBytesToLong(data,
						offset, offset + arraySize);
				bid &= 0xfffffffe;
				// get the details in this block and
				OffsetIndexItem offsetItem = this.pstFile
						.getOffsetIndexNode(bid);
				in.seek(offsetItem.fileOffset);
	...
	public int read(byte[] output) throws IOException {
		...
		if (this.allData != null) {
			...
			if (output.length >= bytesRemaining) {
				System.arraycopy(this.allData, (int) this.currentLocation,
						output, 0, bytesRemaining);
				if (this.encrypted) {
		...
		while (!filled) {
			...
			if (nextSkipPoint >= this.currentLocation + bytesRemaining) {
				...
				System.arraycopy(chunk, 0, output, totalBytesFilled,
						bytesRemaining);
	...
	public Long[] getBlockOffsets() {
		if (this.skipPoints.size() == 0) {
			...
			for (int x = 0; x < output.length; x++) {
				output[x] = new Long(this.skipPoints.get(x)
						+ this.indexItems.get(x).size);
			}
			return output;
		}
	}
	...
	public void seek(long location) throws IOException, PSTException {
		...
		if (location > this.length) {
			throw new PSTException("Unable to seek past end of item! size = "
					+ this.length + ", seeking to:" + location);
		}
	...
	public long seekAndReadLong(long location, int bytes) throws IOException,
			PSTException {
		this.seek(location);

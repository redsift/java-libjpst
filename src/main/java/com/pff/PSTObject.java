...
public class PSTObject {
	 ...
	 */
	protected PSTObject(PSTFile theFile, DescriptorIndexNode folderIndexNode,
			PSTTableBC table,
	...
	public int getNodeType() {
		return PSTObject
				.getNodeType(this.descriptorIndexNode.descriptorIdentifier);
	}
	...
	protected double getDoubleItem(int identifier, double defaultValue) {
		if (this.items.containsKey(identifier)) {
			PSTTableBCItem item = this.items.get(identifier);
			long longVersion = PSTObject
					.convertLittleEndianBytesToLong(item.data);
		...
		if (this.items.containsKey(identifier)) {
			...
			if (item.entryValueType == 0x0003) {
				...
				if (item.data != null && item.data.length == 8) {
					return PSTObject.convertLittleEndianBytesToLong(item.data,
							0, 8);
				} else {
					System.err.printf(
	...
	protected String getStringItem(int identifier, int stringType,
		...
		if (item != null) {
			...
			if (!item.isExternalValueReference) {
				...
				return PSTObject.createJavaString(item.data, stringType,
						codepage);
			}
			if (this.localDescriptorItems != null
					&& this.localDescriptorItems
							.containsKey(item.entryValueReference)) {
				// we have a hit!
				PSTDescriptorItem descItem = this.localDescriptorItems
				...
				try {
					...
					return PSTObject.createJavaString(data, stringType,
							codepage);
				} catch (Exception e) {
					System.err.printf("Exception %s decoding string %s: %s\n",
							e.toString(), PSTFile.getPropertyDescription(
	...
	private String getStringCodepage() {
		...
		if (cpItem != null) {
			return PSTFile
					.getInternetCodePageCharset(cpItem.entryValueReference);
		}
		return null;
	}
	...
	public Date getDateItem(int identifier) {
		if (this.items.containsKey(identifier)) {
			...
			int high = (int) PSTObject.convertLittleEndianBytesToLong(
					item.data, 4, 8);
			int low = (int) PSTObject.convertLittleEndianBytesToLong(item.data,
					0, 4);
			...
			if (item.entryValueType == 0x0102) {
				...
				if (this.localDescriptorItems != null
					...
					try {
						...
						System.err
								.printf("Exception reading binary item: reference 0x%08X\n",
	...
	static int[] compEnc = {  };
	...
	protected static void printHexFormatted(byte[] data, boolean pretty,
		...
		for (int x = 0; x < data.length; x++) {
			...
			if (indexes.length > 0 && x == nextIndex && nextIndex < data.length) {
				System.out.print("+");
				line += "+";
				while (indexIndex < indexes.length - 1
						&& indexes[indexIndex] <= nextIndex) {
					indexIndex++;
				}
	 ...
	 * @return long version of the data
	 */
	public static long convertLittleEndianBytesToLong(byte[] data, int start,
			int end) {
	 ...
	 * @return long version of the data
	 */
	public static long convertBigEndianBytesToLong(byte[] data, int start,
			int end) {
	...
	public static PSTObject detectAndLoadPSTObject(PSTFile theFile,
			long descriptorIndex) throws IOException, PSTException {
		return PSTObject.detectAndLoadPSTObject(theFile,
				theFile.getDescriptorIndexNode(descriptorIndex));
	}
	...
	static PSTObject detectAndLoadPSTObject(PSTFile theFile,
			DescriptorIndexNode folderIndexNode) throws IOException,
			PSTException {
		...
		if (nidType == 0x02 || nidType == 0x03 || nidType == 0x04) {
			...
			if (nidType == 0x02 || nidType == 0x03) {
				return new PSTFolder(theFile, folderIndexNode, table,
						localDescriptorItems);
			} else {
				return PSTObject.createAppropriatePSTMessageObject(theFile,
						folderIndexNode, table, localDescriptorItems);
			}
		} else {
			throw new PSTException("Unknown child type with offset id: "
					+ folderIndexNode.localDescriptorsOffsetIndexIdentifier);
		}
	}

	static PSTMessage createAppropriatePSTMessageObject(PSTFile theFile,
			DescriptorIndexNode folderIndexNode, PSTTableBC table,
			HashMap<Integer, PSTDescriptorItem> localDescriptorItems) {
		...
		if (messageClass.equals("IPM.Note")) {
			return new PSTMessage(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else if (messageClass.equals("IPM.Appointment")
				|| messageClass
						.equals("IPM.OLE.CLASS.{00061055-0000-0000-C000-000000000046}")
				|| messageClass.startsWith("IPM.Schedule.Meeting")) {
			return new PSTAppointment(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else if (messageClass.equals("IPM.Contact")) {
			return new PSTContact(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else if (messageClass.equals("IPM.Task")) {
			return new PSTTask(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else if (messageClass.equals("IPM.Activity")) {
			return new PSTActivity(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else if (messageClass.equals("IPM.Post.Rss")) {
			return new PSTRss(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else if (messageClass.equals("IPM.DistList")) {
			return new PSTDistList(theFile, folderIndexNode, table,
					localDescriptorItems);
		} else {
	...
	static String guessPSTObjectType(PSTFile theFile,
			DescriptorIndexNode folderIndexNode) throws IOException,
			PSTException {
	...
	public static Calendar apptTimeToUTC(int minutes, PSTTimeZone tz) {
		...
		if (tz != null) {
			...
			cLocal.set(cUTC.get(Calendar.YEAR), cUTC.get(Calendar.MONTH),
					cUTC.get(Calendar.DATE), cUTC.get(Calendar.HOUR_OF_DAY),
					cUTC.get(Calendar.MINUTE), cUTC.get(Calendar.SECOND));

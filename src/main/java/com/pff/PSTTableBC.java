	...
	PSTTableBC(PSTNodeInputStream in) throws PSTException, java.io.IOException {
		...
		if (tableTypeByte != 0xffffffbc) {
			...
			throw new PSTException(
					"unable to create PSTTableBC, table does not appear to be a bc!");
		}
		...
		for (int x = 0; x < numberOfKeys; x++) {
			...
			item.itemIndex = x;
			item.entryType = (int) PSTObject.convertLittleEndianBytesToLong(
					keyTableInfo, offset + 0, offset + 2);
			// item.entryType =(int)in.seekAndReadLong(offset, 2);
			item.entryValueType = (int) PSTObject
					.convertLittleEndianBytesToLong(keyTableInfo, offset + 2,
							offset + 4);
			// item.entryValueType = (int)in.seekAndReadLong(offset+2, 2);
			item.entryValueReference = (int) PSTObject
					.convertLittleEndianBytesToLong(keyTableInfo, offset + 4,
							offset + 8);
